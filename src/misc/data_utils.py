...
def subsample_trajectories(expert_states, expert_actions, expert_next_states,
  ...
  """
    ...
    expert_next_states_traj = [
        expert_next_states_traj[i] for i in shuffle_inds
    ]
...
def add_absorbing_states(expert_states, expert_actions, expert_next_states,
  ...
  """
    ...
    expert_next_states = np.pad(expert_next_states, ((0, 0), (0, 1)),
                                mode='constant')
    ...
    expert_logps = [x for x in expert_logps]
    ...
    while i < len(expert_states):
        ...
        if expert_dones[i] and current_len < env._max_episode_steps:  # pylint: disable=protected-access
            ...
            expert_actions.insert(i + 1, np.zeros(
                (env.action_space.shape[0], )))
            expert_dones[i] = np.array([0.0])
            expert_logps[i] = np.array([[0.0]])
            expert_dones.insert(i + 1, np.array([1.0]))
            i += 1
        i += 1
    ...
    expert_logps = np.stack(expert_logps)

    return expert_states, expert_actions, expert_next_states, expert_dones, expert_logps
